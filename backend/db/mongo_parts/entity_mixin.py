...
class EntityMongoMixin:
    ...
    def _find_existing_entity_key(self, entity: Entity) -> Optional[str]:
        """
        Check if an entity with the same display_en_name and entityType already exists.
        Returns its key if found, else None.
        Currently returns False (stub for future enhancement).
        """
        # TODO: implement smarter matching (aliases, fuzzy, etc.)
        doc = self.get_collection(CollectionObjs.ENTITIES).find_one({
            DBFields.DISPLAY_EN_NAME: entity.display_en_name,
            DBFields.ENTITY_TYPE: entity.entityType.value,
        })
        if doc is None:
            return None
