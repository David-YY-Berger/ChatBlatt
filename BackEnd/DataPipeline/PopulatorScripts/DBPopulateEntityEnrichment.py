...
class DBPopulateLmmData(DBParentClass):
    ...
    def test_async_run(self):
        ...
        asyncio.run(self.get_metadata_from_passages())

    async def get_metadata_from_passages(self):
        ...
        for src_content in self.get_examples_src_metadata():
    ...
    ############################################## helper methods ####################################################
    def get_examples_src_metadata(self) -> list[SourceMetadata]:
        ...
        # res = [self.db_api.find_one_source_content(k) for k in key_strs] //create for source metadata
        # return res
        return None
